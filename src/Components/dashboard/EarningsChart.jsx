  ...
  if (active && payload && payload.length) {
    return (
      <div className="glass-strong rounded-lg px-3 py-2">
        ...
        <p className="text-sm font-semibold text-foreground">R${payload[0].value.toLocaleString()}</p>
      </div>
    );
  }
  return null;
...
export default function EarningsChart({ data }) {
  ...
  return (
    <div className="glass rounded-2xl p-5">
      <h3 className="text-sm font-semibold text-foreground mb-4">Ganhos Mensais</h3>
      <div className="h-56">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData}>
            ...
            <Area type="monotone" dataKey="ganhos" stroke="hsl(258 70% 58%)" strokeWidth={2} fill="url(#earningsGrad)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
